...
//set the total number of frequency x angles
  ...
  //!    but not Hydro.
  //! 3. MAGNETIC_FIELDS_ENABLED, SELF_GRAVITY_ENABLED, NDUSTFLUIDS, NSCALARS, (future) FLUID_ENABLED,
  ...
  // class type that is used in each Dust Fluids, PassiveScalars, Gravity, Field, Hydro, ... etc. class
  ...
  // OrbitalAdvection: constructor depends on Coordinates, Hydro, Field, Dust Fluids, PassiveScalars.
...
MeshBlock::MeshBlock(int igid, int ilid, Mesh *pm, ParameterInput *pin,
  ...
  if (NDUSTFLUIDS > 0) {
    std::memcpy(pdustfluids->df_u.data(), &(mbdata[os]), pdustfluids->df_u.GetSizeInBytes());
    // load it into the other memory register(s) too
    std::memcpy(pdustfluids->df_u1.data(), &(mbdata[os]), pdustfluids->df_u1.GetSizeInBytes());
    os += pdustfluids->df_u.GetSizeInBytes();
    if (pdustfluids->dfdif.dustfluids_diffusion_defined) {
...
std::size_t MeshBlock::GetBlockSizeInBytes() {
  ...
  if (NDUSTFLUIDS > 0) {
    size += pdustfluids->df_u.GetSizeInBytes();
    if (pdustfluids->dfdif.dustfluids_diffusion_defined)
