...
//! (not primitive) variable standard/coarse arrays for Hydro, Dust Fluids, PassiveScalars
//!
...
//! 1. AthenaArray<Real> DustFluids::coarse_df_w, PassiveScalars::coarse_r
...
void MeshRefinement::SetHydroRefinement(HydroBoundaryQuantity hydro_type) {
  //! \todo (felker):
  //! * make more general so it can be used as SetPassiveScalarsRefinement() and SetDustFluidsRefinement()
...
void MeshRefinement::SetDustFluidsRefinement(DustFluidsBoundaryQuantity dustfluids_type) {
  ...
  switch (dustfluids_type) {
    case (DustFluidsBoundaryQuantity::cons_df): {
      pvars_cc_.front() = std::make_tuple(&pdf->df_u, &pdf->coarse_df_u_);
      break;
    }
    case (DustFluidsBoundaryQuantity::prim_df): {
      pvars_cc_.front() = std::make_tuple(&pdf->df_w, &pdf->coarse_df_w_);
      break;
    }
  }
  return;
