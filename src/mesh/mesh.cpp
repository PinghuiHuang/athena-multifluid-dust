...
#pragma omp for private(pmb,pbval)
      for (int i=0; i<nblocal; ++i) {
        ...
        if (NDUSTFLUIDS > 0) {
          ...
          pmb->pdustfluids->dfbvar.SwapDustFluidsQuantity(pmb->pdustfluids->df_u,
...
#pragma omp for private(pmb,pbval)
        for (int i=0; i<nblocal; ++i) {
          ...
          pmb->phydro->hbvar.SendBoundaryBuffers();
          if (NDUSTFLUIDS > 0) {
            pmb->pdustfluids->dfbvar.SwapDustFluidsQuantity(pmb->pdustfluids->df_w,
...
#pragma omp for private(pmb)
      ...
      DustFluids *pdf;
      PassiveScalars *ps;
#pragma omp for private(pmb,pbval,ph,pf,pdf,ps)
      for (int i=0; i<nblocal; ++i) {
        pmb = my_blocks(i);
        ...
        if (NDUSTFLUIDS > 0) {
          pmb->peos->DustFluidsConservedToPrimitive(pdf->df_u, pdf->dfccdif.diff_mom_cc,
                          pdf->df_w1, pdf->df_w, pmb->pcoord, il, iu, jl, ju, kl, ku);
        }
        if (NSCALARS > 0) {
          ...
          if (NDUSTFLUIDS > 0) {
            ...
            if (dust_xorder == 4)
              pmb->peos->DustFluidsConservedToPrimitiveCellAverage(
                  pdf->df_u, pdf->df_w1, pdf->df_w, pmb->pcoord, il, iu, jl, ju, kl, ku);
          }
          if (NSCALARS > 0) {
        ...
        // Swap Hydro and (possibly) dust fluids, passive scalar quantities in BoundaryVariable
...
#pragma omp for private(pmb,ph,pf,pdf)
      for (int i=0; i<nblocal; ++i) {
        ...
        if (NDUSTFLUIDS > 0) { // Calculate the initial dust stopping time, diffusivity and dust sound speed
          pdf->SetDustFluidsProperties(pmb->pmy_mesh->time, ph->w, pdf->df_w,
...
#pragma omp for private(pmb, ph, pdf, ps)
  for (int nb=0; nb<nblocal; ++nb) {
    ...
    AthenaArray<Real> delta_cons_, delta_df_u_, delta_s_;
    ...
    if (NDUSTFLUIDS > 0) {
      delta_df_u_.NewAthenaArray(ncells4_df, pmb->ncells3, pmb->ncells2, pmb->ncells1);
      pmb->pcoord->Laplacian(pdf->df_u, delta_df_u_, il, iu, jl, ju, kl, ku, nl_df, nu_df);
    }
    ...
    for (int n=nl_df; n<=nu_df; ++n) {
      for (int k=kl; k<=ku; ++k) {
        for (int j=jl; j<=ju; ++j) {
          for (int i=il; i<=iu; ++i) {
            pdf->df_u(n,k,j,i) = pdf->df_u(n,k,j,i) + C*delta_df_u_(n,k,j,i);
          }
        }
      }
    }
