...
class EquationOfState {
 ...
 public:
  ...
  void DustFluidsPrimitiveToConserved(
    ...
    AthenaArray<Real> &cons_df, Coordinates *pco, int il, int iu, int jl, int ju, int kl, int ku);
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
 ...
 private:
  ...
  Real dustfluids_floor_[NDUSTGAS];      // dust fluids density floor, I set dustfluids_floor_ as NDUSTGAS, so that it still can be initilized when NDUSTFLUIDS == 0.
  ...
  Real dustfluids_rho_min_[NDUSTGAS];    // variables to control power-law dust denity floor
  Real dustfluids_rho_pow_[NDUSTGAS];    // variables to control power-law dust denity floor
