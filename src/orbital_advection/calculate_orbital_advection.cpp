...
//!                            AthenaArray<Real> &u, AthenaArray<Real> &u_df, AthenaArray<Real> &s)
//! \brief update hydro, dust fluids & passive scalars using orbital advection scheme
        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if (NDUSTFLUIDS > 0) {
            for (int ndv=0; ndv<NDUSTVARS; ++ndv) {
              dfbuf.InitWithShallowSlice(orbital_df_u, 4, ndv, 1);
              if (dust_xorder <= 2) {
              ...
              for (int j=js; j<=je; j++) {
                u_df(ndv,k,j,i) = dfbuf(k,i,j+shift) - (pflux(j+1) - pflux(j));
              }
            }
          }
        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if (NDUSTFLUIDS > 0) {
            for (int ndv=0; ndv<NDUSTVARS; ++ndv) {
              ...
              for (int k=ks; k<=ke; k++) {
                u_df(ndv,k,j,i) = dfbuf(j,i,k+shift) - (pflux(k+1) - pflux(k));
              }
            }
          }
          // calculate passive scalar flux
