...
void OrbitalAdvection::SetOrbitalAdvectionCC(const AthenaArray<Real> &u,
               const AthenaArray<Real> &u_df, const AthenaArray<Real> &s) {
...
#pragma omp simd
            for (int j = jl; j <= ju; j++) {
              orbital_df_u(ndv,k,i,j+shift)= u_df(ndv,k,j,i);
            }
          }
        }
      }
...
#pragma omp simd
            for (int k = kl; k <= ku; k++) {
              orbital_df_u(ndv,j,i,k+shift)= u_df(ndv,k,j,i);
            }
          }
        }
      }
...
#pragma omp simd
  ...
  if (orbital_refinement) {
    ...
    if (NDUSTFLUIDS>0)
      pmb_->pmr->RestrictCellCenteredValues(u_df, df_u_coarse_send, 0, NDUSTVARS-1,
                                            pmb_->cis, pmb_->cie, pmb_->cjs,
                                            pmb_->cje, pmb_->cks, pmb_->cke);
    if (NSCALARS>0)
