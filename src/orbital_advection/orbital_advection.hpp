...
class OrbitalAdvection{
 ...
 public:
  ...
  void SetOrbitalAdvectionCC(const AthenaArray<Real> &u, const AthenaArray<Real> &u_df, const AthenaArray<Real> &s);
  void SetOrbitalAdvectionFC(const FaceField &b);
  void CalculateOrbitalAdvectionCC(const Real dt, AthenaArray<Real> &u,
                     AthenaArray<Real> &u_df, AthenaArray<Real> &s);
  ...
  void ConvertOrbitalSystemDustFluids(const int dust_id, const AthenaArray<Real> &df_w0,
  ...
  AthenaArray<Real> df_w_orb, df_u_orb; // buffer for orbital advection system output for dust fluids
 ...
 private:
  ...
  AthenaArray<Real> orbital_cons, orbital_df_u, orbital_scalar;
  ...
  AthenaArray<Real> df_u_coarse_send, df_u_coarse_recv, df_u_temp;
