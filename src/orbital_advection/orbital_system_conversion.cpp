...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalAdvection::ConvertOrbitalSystemDustFluids(const AthenaArray<Real> &df_w0,
//!                            const AthenaArray<Real> &df_u0, const OrbitalTransform trans)
//! \brief convert the orbital system to the normal system

void OrbitalAdvection::ConvertOrbitalSystemDustFluids(const int dust_id, const AthenaArray<Real> &df_w0,
                       const AthenaArray<Real> &df_u0, const OrbitalTransform trans) {
  int flag = static_cast<int>(trans);
  if ((orbital_system_conversion_done&flag) > 0) {
    int il = pmb_->is-(NGHOST); int jl = pmb_->js; int kl = pmb_->ks;
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              df_w_orb(rho_id, k, j, i) = df_w0(rho_id, k, j, i);
              df_w_orb(v1_id,  k, j, i) = df_w0(v1_id,  k, j, i);
              df_w_orb(v2_id,  k, j, i) = df_w0(v2_id,  k, j, i) + vKc(k, i);
              df_w_orb(v3_id,  k, j, i) = df_w0(v3_id,  k, j, i);
            }
          }
        }
      } else if(orbital_direction == 2) {
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              df_w_orb(rho_id, k, j, i) = df_w0(rho_id, k, j, i);
              df_w_orb(v1_id,  k, j, i) = df_w0(v1_id,  k, j, i);
              df_w_orb(v2_id,  k, j, i) = df_w0(v2_id,  k, j, i);
              df_w_orb(v3_id,  k, j, i) = df_w0(v3_id,  k, j, i) + vKc(j, i);
            }
          }
        }
      }
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              Real den = df_u0(rho_id, k, j, i);
              Real m2  = df_u0(v2_id,  k, j, i);
              Real mk  = den*vKc(k, i);
              df_u_orb(rho_id, k, j, i) = den;
              df_u_orb(v1_id,  k, j, i) = df_u0(v1_id, k, j, i);
              df_u_orb(v2_id,  k, j, i) = m2 + mk;
              df_u_orb(v3_id,  k, j, i) = df_u0(v3_id, k, j, i);
            }
					}
				}
      } else if(orbital_direction == 2) {
...
#pragma omp simd
            for(int i=il; i<=iu; i++) {
              Real den = df_u0(rho_id, k, j, i);
              Real m3  = df_u0(v3_id,  k, j, i);
              Real mk  = den*vKc(j, i);
              df_u_orb(rho_id, k, j, i) = den;
              df_u_orb(v1_id,  k, j, i) = df_u0(v1_id, k, j, i);
              df_u_orb(v2_id,  k, j, i) = df_u0(v2_id, k, j, i);
              df_u_orb(v3_id,  k, j, i) = m3+ mk;
            }
          }
        }
      }
