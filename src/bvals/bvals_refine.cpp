...
//! ProlongateBoundaries expects to have Hydro (and dust fluids, passive scalar)-associated
...
//! (u, coarse_cons) for Hydro, (df_u, coarse_df_u) for DustFluids and (s, coarse_s) for PassiveScalars
...
void BoundaryValues::ProlongateBoundaries(const Real time, const Real dt,
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    //if (pdf->dfdif.dustfluids_diffusion_defined)
      //pdfccdifbvar = dynamic_cast<DustDiffusionBoundaryVariable *>(bvars_main_int[2]);
    //pdfbvar = dynamic_cast<DustFluidsBoundaryVariable *>(bvars_main_int[3]); // TODO:P.H.
  }
  ...
  for (int n=0; n<nneighbor; n++) {
    ...
    if (NDUSTFLUIDS > 0) {
      ...
      pdf->dfbvar.var_cc = &(pdf->coarse_df_w_);
    }
    if (NSCALARS > 0) {
      ps = pmb->pscalars;
...
void BoundaryValues::ApplyPhysicalBoundariesOnCoarseLevel(
  ...
  if (NDUSTFLUIDS > 0) {
    pmb->peos->DustFluidsConservedToPrimitive(pdf->coarse_df_u_, pdf->dfccdif.coarse_diff_mom_cc_,
                                              pdf->coarse_df_w_, pdf->coarse_df_w_,
                                              pmr->pcoarsec,
  ...
  if (nb.ni.ox1 == 0) {
    if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt,
                                ...
                                ph->coarse_prim_, pdf->coarse_df_w_,
                                pf->coarse_b_,
...
void BoundaryValues::ProlongateGhostCells(const NeighborBlock& nb,
  ...
  //(unique to Hydro, DustFluids, PassiveScalars): swap ptrs to (w, coarse_prim) from (u, coarse_cons)
  pmr->SetHydroRefinement(HydroBoundaryQuantity::prim);
  // (df_w, coarse_df_w) from (df_u, coarse_df_u)
  if (NDUSTFLUIDS > 0) {
    DustFluids *pdf = pmb->pdustfluids;
    //pmr->SetDustFluidsRefinement(DustFluidsBoundaryQuantity::prim_df);
    pmr->pvars_cc_[pdf->refinement_idx] = std::make_tuple(&pdf->df_w, &pdf->coarse_df_w_);
  }
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    pmr->pvars_cc_[pdf->refinement_idx] = std::make_tuple(&pdf->df_u, &pdf->coarse_df_u_);
  }
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
  ...
  if (NDUSTFLUIDS > 0) {
    DustFluids *pdf = pmb->pdustfluids;
    pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                              pdf->df_u, pmb->pcoord, fsi, fei, fsj, fej, fsk, fek);
  }
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
