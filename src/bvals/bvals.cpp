...
BoundaryValues::BoundaryValues(MeshBlock *pmb, BoundaryFlag *input_bcs,
  ...
  // std::vector<BoundaryVariable *>.push_back() in Hydro, Field, Gravity, DustFluids, PassiveScalar
  bvars.reserve(5);
...
void BoundaryValues::ApplyPhysicalBoundaries(const Real time, const Real dt,
  ...
  if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
    ...
    if (NDUSTFLUIDS > 0) {
      pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                      pdf->df_u, pco, pmb->is-NGHOST, pmb->is-1, bjs, bje, bks, bke);
    }
    if (NSCALARS > 0) {
  ...
  if (apply_bndry_fn_[BoundaryFace::outer_x1]) {
    ...
    if (NDUSTFLUIDS > 0) {
      pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                      pdf->df_u, pco, pmb->ie+1, pmb->ie+NGHOST, bjs, bje, bks, bke);
    }
    if (NSCALARS > 0) {
      pmb->peos->PassiveScalarPrimitiveToConserved(
  ...
  if (pmb->block_size.nx2 > 1) { // 2D or 3D
    ...
    if (apply_bndry_fn_[BoundaryFace::inner_x2]) {
      ...
      if (NDUSTFLUIDS > 0) {
        pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                        pdf->df_u, pco, bis, bie, pmb->js-NGHOST, pmb->js-1, bks, bke);
      }
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
    ...
    if (apply_bndry_fn_[BoundaryFace::outer_x2]) {
      ...
      if (NDUSTFLUIDS > 0) {
        pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                        pdf->df_u, pco, bis, bie, pmb->je+1, pmb->je+NGHOST, bks, bke);
      }
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
  ...
  if (pmb->block_size.nx3 > 1) { // 3D
    ...
    if (apply_bndry_fn_[BoundaryFace::inner_x3]) {
      ...
      if (NDUSTFLUIDS > 0) {
        pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                        pdf->df_u, pco, bis, bie, bjs, bje, pmb->ks-NGHOST, pmb->ks-1);
      }
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
    ...
    if (apply_bndry_fn_[BoundaryFace::outer_x3]) {
      ...
      if (NDUSTFLUIDS > 0) {
        pmb->peos->DustFluidsPrimitiveToConserved(pdf->df_w, pdf->dfccdif.diff_mom_cc,
                        pdf->df_u, pco, bis, bie, bjs, bje, pmb->ke+1, pmb->ke+NGHOST);
      }
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
