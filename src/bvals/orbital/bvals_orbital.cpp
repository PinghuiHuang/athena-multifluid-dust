...
//! \brief load and send hydro variables, dust fluids, passive scalars
...
//! \brief receive and set hydro variables, dust fluids and passive scalars
...
void OrbitalBoundaryCommunication::LoadDustFluidsBufferSameLevel(
  ...
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &dfi = pmb->pdustfluids->df_u;
...
void OrbitalBoundaryCommunication::LoadDustFluidsBufferToCoarser(Real *buf, int &p, int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &dfi = porb->df_u_coarse_send;
...
void OrbitalBoundaryCommunication::SetDustFluidsBufferSameLevel(
  ...
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &df_uo = porb->orbital_df_u;
...
void OrbitalBoundaryCommunication::SetDustFluidsBufferFromCoarser(
  ...
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &df_uo = porb->orbital_df_u;
  AthenaArray<Real> &dfco = porb->df_u_coarse_recv;
  AthenaArray<Real> &dfto = porb->df_u_temp;
      ...
      if(nb==0) {
        ...
        for(int k=pmb->ks; k<=pmb->ke; k++) {
          for(int i=pmb->is; i<=pmb->ie; i++) {
            ...
            for(int ndv=0 ; ndv<NDUSTVARS; ndv++) {
              for(int j=xl; j<=xu; j++) {
                df_uo(ndv,k,i,j+shift) = dfto(ndv,k,j,i);
              }
            }
          }
        }
      ...
      if(nb==0) {
        ...
        for(int j=pmb->js; j<=pmb->je; j++) {
          for(int i=pmb->is; i<=pmb->ie; i++) {
            ...
            for(int ndv=0 ; ndv<NDUSTVARS; ndv++) {
              for (int k=xl; k<=xu; k++) {
                df_uo(ndv,j,i,k+shift) = dfto(ndv,k,j,i);
              }
            }
          }
        }
