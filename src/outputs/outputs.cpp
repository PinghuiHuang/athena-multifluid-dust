  ...
  while (pib != nullptr) {
    if (pib->block_name.compare(0, 6, "output") == 0) {
      ...
      if (op.dt > 0.0 || op.dcycle > 0) {  // only add output if dt > 0 or dycle > 0
        ...
        // read avg options.  Check for conflicts with slicing.
        op.output_avgx1 = pin->GetOrAddBoolean(op.block_name,"x1_avg",false);
        if ((op.output_slicex1) && (op.output_avgx1)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and avg along x1-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }
        op.output_avgx2 = pin->GetOrAddBoolean(op.block_name,"x2_avg",false);
        if ((op.output_slicex2) && (op.output_avgx2)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and avg along x2-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }
        op.output_avgx3 = pin->GetOrAddBoolean(op.block_name,"x3_avg",false);
        if ((op.output_slicex3) && (op.output_avgx3)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and avg along x3-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }

        // read max options.  Check for conflicts with slicing.
        op.output_maxx1 = pin->GetOrAddBoolean(op.block_name,"x1_max",false);
        if ((op.output_slicex1) && (op.output_maxx1)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and max along x1-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }
        op.output_maxx2 = pin->GetOrAddBoolean(op.block_name,"x2_max",false);
        if ((op.output_slicex2) && (op.output_maxx2)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and max along x2-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }
        op.output_maxx3 = pin->GetOrAddBoolean(op.block_name,"x3_max",false);
        if ((op.output_slicex3) && (op.output_maxx3)) {
          msg << "### FATAL ERROR in Outputs constructor" << std::endl
              << "Cannot request both slice and max along x3-direction"
              << " in output block '" << op.block_name << "'" << std::endl;
          ATHENA_ERROR(msg);
        }
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    for (int n=0; n<NDUSTFLUIDS; n++) {
      ...
      if (ContainVariable(output_params.variable, dust_den_str) ||
        ...
        pod->data.InitWithShallowSlice(pdfs->df_u, 4, rho_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_mom_str) ||
        ...
        if(porb->orbital_advection_defined && !output_params.orbital_system_output) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->df_u_orb, 4, v1_id, 3);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_u, 4, v1_id, 3);
        }
        AppendOutputDataNode(pod);
        num_vars_ += 3;
        if (output_params.cartesian_vector) {
          ...
          if(porb->orbital_advection_defined
             && !output_params.orbital_system_output) {
            src.InitWithShallowSlice(porb->df_u_orb, 4, v1_id, 3);
          } else {
            src.InitWithShallowSlice(pdfs->df_u, 4, v1_id, 3);
          }
          pod = new OutputData;
          pod->type = "VECTORS";
          pod->name = dust_mom_xyz_str;
          pod->data.NewAthenaArray(3, pdfs->df_u.GetDim3(), pdfs->df_u.GetDim2(),
                                   pdfs->df_u.GetDim1());
          CalculateCartesianVector(src, pod->data, pmb->pcoord);
          AppendOutputDataNode(pod);
          num_vars_ += 3;
      ...
      if (ContainVariable(output_params.variable, dust_mom1_str)) {
        ...
        pod->data.InitWithShallowSlice(pdfs->df_u, 4, v1_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_mom2_str)) {
        ...
        pod->data.InitWithShallowSlice(pdfs->df_u, 4, v2_id, 1);
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
           && porb->orbital_direction == 1) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->df_u_orb, 4, v2_id, 1);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_u, 4, v2_id, 1);
        }
        AppendOutputDataNode(pod);
      ...
      if (ContainVariable(output_params.variable, dust_mom3_str)) {
        ...
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
          ...
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::cons);
          pod->data.InitWithShallowSlice(porb->df_u_orb, 4, v3_id, 1);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_u, 4, v3_id, 1);
        }
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_rho_str) ||
        ...
        pod->data.InitWithShallowSlice(pdfs->df_w, 4, rho_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_vel_str) ||
        ...
        if(porb->orbital_advection_defined && !output_params.orbital_system_output) {
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::prim);
          pod->data.InitWithShallowSlice(porb->df_w_orb, 4, v1_id, 3);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_w, 4, v1_id, 3);
        }
        AppendOutputDataNode(pod);
        num_vars_ += 3;
        if (output_params.cartesian_vector) {
          ...
          if(porb->orbital_advection_defined
             && !output_params.orbital_system_output) {
            src.InitWithShallowSlice(porb->df_w_orb, 4, v1_id, 3);
          } else {
            src.InitWithShallowSlice(pdfs->df_w, 4, v1_id, 3);
          }
          pod = new OutputData;
          pod->type = "VECTORS";
          pod->name = dust_vel_xyz_str;
          pod->data.NewAthenaArray(3, pdfs->df_w.GetDim3(), pdfs->df_w.GetDim2(),
                                   pdfs->df_w.GetDim1());
          CalculateCartesianVector(src, pod->data, pmb->pcoord);
          AppendOutputDataNode(pod);
          num_vars_ += 3;
      ...
      if (ContainVariable(output_params.variable, dust_vel1_str)) {
        ...
        pod->data.InitWithShallowSlice(pdfs->df_w, 4, v1_id, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_vel2_str)) {
        ...
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
          ...
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::prim);
          pod->data.InitWithShallowSlice(porb->df_w_orb, 4, v2_id, 1);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_w, 4, v2_id, 1);
        }
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      ...
      if (ContainVariable(output_params.variable, dust_vel3_str)) {
        ...
        if(porb->orbital_advection_defined && !output_params.orbital_system_output
          ...
          porb->ResetOrbitalSystemConversionFlag();
          porb->ConvertOrbitalSystemDustFluids(dust_id, pdfs->df_w, pdfs->df_u, OrbitalTransform::prim);
          pod->data.InitWithShallowSlice(porb->df_w_orb, 4, v3_id, 1);
        } else {
          pod->data.InitWithShallowSlice(pdfs->df_w, 4, v3_id, 1);
        }
        AppendOutputDataNode(pod);
        num_vars_++;
      }
...
//----------------------------------------------------------------------------------------
//! \fn void OutputType::AvgOutputData(OutputData* pod, int dim)
//! \brief perform data average and update the data list

void OutputType::AvgOutputData(MeshBlock* pmb, int dim) {
  // For each node in OutputData doubly linked list, average arrays containing output data
  OutputData *pdata = pfirst_data_;
  ...
  while (pdata != nullptr) {
    ...
    Real inv_k_number = 1.0/(pmb->ke - pmb->ks + 1.);
    Real inv_j_number = 1.0/(pmb->je - pmb->js + 1.);
    Real inv_i_number = 1.0/(pmb->ie - pmb->is + 1.);

    // Loop over variables and dimensions, average over specified dimension
    if (dim == 3) {
      pnew->data.NewAthenaArray(nx4, 1, nx2, nx1);
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n, 0, j, i) += pdata->data(n, k, j, i)*inv_k_number;
            }
          }
        }
      }
      ...
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n, k, 0, i) += pdata->data(n, k, j, i)*inv_j_number;
            }
          }
        }
      }
      ...
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n, k, j, 0) += pdata->data(n, k, j, i)*inv_i_number;
            }
          }
        }
      }
...
//
//----------------------------------------------------------------------------------------
//! \fn void OutputType::MaxOutputData(OutputData* pod, int dim)
//! \brief perform data maximum and update the data list

void OutputType::MaxOutputData(MeshBlock* pmb, int dim) {
  // For each node in OutputData doubly linked list, sum arrays containing output data
  OutputData *pdata = pfirst_data_;
    ...
    if (dim == 3) {
      ...
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n,0,j,i) = std::max(pnew->data(n,0,j,i), pdata->data(n,k,j,i));
            }
          }
        }
      }
      ...
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n,k,0,i) = std::max(pnew->data(n,k,0,i), pdata->data(n,k,j,i));
            }
          }
        }
      }
      ...
      for (int n=0; n<nx4; ++n) {
        for (int k=out_ks; k<=out_ke; ++k) {
          for (int j=out_js; j<=out_je; ++j) {
            for (int i=out_is; i<=out_ie; ++i) {
              pnew->data(n,k,j,0) = std::max(pnew->data(n,k,j,0), pdata->data(n,k,j,i));
            }
          }
        }
      }
