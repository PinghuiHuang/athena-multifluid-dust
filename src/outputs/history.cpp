...
#define NHISTORY_VARS ((NHYDRO) + (SELF_GRAVITY_ENABLED > 0) + (NFIELD) + 3 + (NDUSTVARS) + (NSCALARS) \
                      +(NRAD) + (NCR))
        ...
        for (int j=pmb->js; j<=pmb->je; ++j) {
          ...
          for (int i=pmb->is; i<=pmb->ie; ++i) {
            ...
            for (int n=0; n<NDUSTVARS; n++) {
              const Real& df_u = porb->df_u_orb(n,k,j,i);
              constexpr int prev_out = NHYDRO + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD;
              hst_data[prev_out + n] += vol(i)*df_u;
            }
            // (conserved variable) Passive scalars:
            for (int n=0; n<NSCALARS; n++) {
              const Real& s = psclr->s(n,k,j,i);
              constexpr int prev_out = NHYDRO + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD + NDUSTVARS;
      ...
      for (int k=pmb->ks; k<=pmb->ke; ++k) {
        for (int j=pmb->js; j<=pmb->je; ++j) {
          ...
          for (int i=pmb->is; i<=pmb->ie; ++i) {
            ...
            // (conserved variable) Dust Fluids:
            for (int n=0; n<NDUSTVARS; n++) {
              const Real& df_u = pdfs->df_u(n,k,j,i);
              constexpr int prev_out = NHYDRO + 3 + (SELF_GRAVITY_ENABLED > 0) + NFIELD;
              hst_data[prev_out + n] += vol(i)*df_u;
            }
            // (conserved variable) Passive scalars:
