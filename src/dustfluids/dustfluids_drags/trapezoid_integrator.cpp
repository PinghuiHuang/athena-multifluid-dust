...
#pragma omp simd
        for (int i=is; i<=ie; ++i) {
          ...
          delta_mom1_src(0, i) = 0.5*(u_af_src(IM1, k, j, i) + delta_mom1(0, i) - gas_mom1_bf_src);
          delta_mom2_src(0, i) = 0.5*(u_af_src(IM2, k, j, i) + delta_mom2(0, i) - gas_mom2_bf_src);
          delta_mom3_src(0, i) = 0.5*(u_af_src(IM3, k, j, i) + delta_mom3(0, i) - gas_mom3_bf_src);
...
#pragma omp simd
        for (int i=is; i<=ie; ++i) {
          ...
          Real gas_delta_mom1_drag = 2.0*(delta_mom1_im(0, i) - delta_mom1_src(0, i));
          Real gas_delta_mom2_drag = 2.0*(delta_mom2_im(0, i) - delta_mom2_src(0, i));
          Real gas_delta_mom3_drag = 2.0*(delta_mom3_im(0, i) - delta_mom3_src(0, i));
...
#pragma omp simd
          for (int i=is; i<=ie; ++i) {
            ...
            Real dust_delta_mom3_drag = 2.0*(delta_mom3_im(idx, i) - delta_mom3_src(idx, i));
