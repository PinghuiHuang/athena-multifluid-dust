...
//----------------------------------------------------------------------------------------
// Coordinate (Geometric) source term function for dust fluids

void Coordinates::AddDustFluidsCoordTermsDivergence(const Real dt, const AthenaArray<Real> *df_flx,
                                   const AthenaArray<Real> &prim_df, AthenaArray<Real> &u_df) {
  bool use_x2_fluxes = pmy_block->block_size.nx2 > 1;
...
#pragma omp simd
        for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
          ...
          if (pdf->SoundSpeed_Flag)
            m_ii += 2.0*SQR(pdf->cs_dustfluids_array(dust_id, k, j, i)) * prim_df(rho_id, k, j, i);
          ...
          u_df(v1_id, k, j, i) += dt*coord_src1_i_(i)*m_ii;

          // src_2 = -< M_{theta r} ><1/r>
          u_df(v2_id, k, j, i) -= dt*coord_src2_i_(i)*
                          (coord_area1_i_(i)   * df_flx[X1DIR](v2_id, k, j, i)
                          +coord_area1_i_(i+1) * df_flx[X1DIR](v2_id, k, j, i+1));

          // src_3 = -< M_{phi r} ><1/r>
          u_df(v3_id,k,j,i) -= dt*coord_src2_i_(i)*
          ...
          u_df(v2_id, k, j, i) += dt*coord_src1_i_(i)*coord_src1_j_(j)*m_pp;

          // src_3 = -< M_{phi theta} ><cot theta/r>
          if (use_x2_fluxes) {
            u_df(v3_id, k, j, i) -= dt*coord_src1_i_(i)*coord_src2_j_(j)*
          ...
          if (!STS_ENABLED) {
            ...
            u_df(v3_id, k, j, i) -= dt*coord_src1_i_(i)*coord_src3_j_(j)*m_ph;
          }
