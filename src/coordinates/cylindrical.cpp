...
void Coordinates::AddDustFluidsCoordTermsDivergence(
...
#pragma omp simd
        for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
          ...
          u_df(v1_id,k,j,i) += dt*coord_src1_i_(i)*m_pp;

          // src_2 = -< M_{phi r} ><1/r>
          const Real& x_i   = x1f(i);
          const Real& x_ip1 = x1f(i+1);
          // Ju PhD thesis equation 2.14
          u_df(v2_id, k, j, i) -= dt*coord_src2_i_(i)*(x_i*df_flux[X1DIR](v2_id, k, j, i)
