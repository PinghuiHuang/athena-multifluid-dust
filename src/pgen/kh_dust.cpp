...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  //dffloor = pin->GetOrAddReal("dust","dffloor",1e-6);
  ...
  if (iprob == 4 && NDUSTFLUIDS > 0 && NSCALARS > 0) {
    AllocateUserHistoryOutput(2);
    EnrollUserHistoryOutput(0, DustFluidsDyeEntropy, "tot-D");
    EnrollUserHistoryOutput(1, PassiveDyeEntropy, "tot-S");
  }
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          ...
          // color concentration of dust fluids
          if (NDUSTFLUIDS > 0) {
            Real concentration = 0.5*(std::tanh((pcoord->x2v(j) - z2)/a)  // 8e)
                                    - std::tanh((pcoord->x2v(j) - z1)/a) + 2.0);
            //concentration = concentration > dffloor ? concentration : dffloor;
            for (int n = 0; n<NDUSTFLUIDS; n++) {
              int dust_id = n;
              ...
              int v2_id   = rho_id + 2;
              int v3_id   = rho_id + 3;
              pdustfluids->df_u(rho_id, k, j, i) = concentration*phydro->u(IDN, k, j, i);
              pdustfluids->df_u(v1_id,  k, j, i) = 0;
              pdustfluids->df_u(v2_id,  k, j, i) = 0;
              pdustfluids->df_u(v3_id,  k, j, i) = 0;
            }
          }
          ...
          if (NSCALARS > 0) {
            ...
            for (int n=0; n<NSCALARS; ++n) {
              pscalars->s(n,k,j,i) = concentration*phydro->u(IDN,k,j,i);
            }
          }
        }
      }
        ...
        for (int i=is; i<=ie; i++) {
          ...
          if (NDUSTFLUIDS > 0) {
            ...
            for (int n = 0; n<NDUSTFLUIDS; n++) {
              ...
              int v2_id   = rho_id + 2;
              int v3_id   = rho_id + 3;
              pdustfluids->df_u(rho_id, k, j, i) = initial_D2G[dust_id] * phydro->u(IDN, k, j, i);
              pdustfluids->df_u(v1_id,  k, j, i) = initial_D2G[dust_id] * phydro->u(IM1, k, j, i);
              pdustfluids->df_u(v2_id,  k, j, i) = initial_D2G[dust_id] * phydro->u(IM2, k, j, i);
              pdustfluids->df_u(v3_id,  k, j, i) = initial_D2G[dust_id] * phydro->u(IM3, k, j, i);
            }
          }
          ...
          if (NSCALARS > 0) {
            ...
            //concentration = concentration > dffloor ? concentration : dffloor;
            for (int n = 0; n<NSCALARS; n++) {
              pscalars->s(n, k, j, i) = (n+1.)*phydro->u(IDN, k, j, i);
            }
          }
        }
      }
...
int RefinementCondition(MeshBlock *pmb) {
  ...
  AthenaArray<Real> &prim_df = pmb->pdustfluids->df_w;
  Real vgmax = 0.0;
  Real vdmax = 0.0;
      ...
      for (int i=pmb->is-1; i<=pmb->ie+1; i++) {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            vdy = std::abs(prim_df(v2_id,k,j,i+1) - prim_df(v2_id,k,j,i-1))*0.5;
            vdx = std::abs(prim_df(v1_id,k,j+1,i) - prim_df(v1_id,k,j-1,i))*0.5;
            vd  = std::sqrt(vdx*vdx + vdy*vdy);
          }
  ...
  if (std::min(vgmax, vdmax) > threshold) return 1;
  if (std::max(vgmax, vdmax) < 0.5*threshold) return -1;
  return 0;
}
    ...
    for (int j=js; j<=je; j++) {
      ...
      for (int i=is; i<=ie; i++) {
        ...
        for (int n=0; n<NDUSTFLUIDS; n++) {
          int dust_id = n;
          int rho_id  = 4*dust_id;
          Real specific_entropy = -df_w(rho_id,k,j,i)/w(IDN,k,j,i)
            *std::log(df_w(rho_id,k,j,i)/w(IDN,k,j,i));
