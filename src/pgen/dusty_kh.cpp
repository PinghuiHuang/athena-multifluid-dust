      ...
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          ...
          if (NDUSTFLUIDS > 0) {
            ...
            concentration = std::max(concentration, dffloor);
    ...
    for (int j=js; j<=je; j++) {
      ...
      for (int i=is; i<=ie; i++) {
        ...
        for (int n=0; n<NDUSTFLUIDS; n++) {
          int dust_id = n;
          int rho_id  = 4*dust_id;
          Real specific_entropy = -df_w(rho_id,k,j,i)/(df_w(rho_id,k,j,i)+w(IDN,k,j,i))
            *std::log(df_w(rho_id,k,j,i)/(df_w(rho_id,k,j,i)+w(IDN,k,j,i)));
