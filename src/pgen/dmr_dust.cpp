...
void DMRInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, AthenaArray<Real> &prim_df,
                FaceField &b, Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh);
void DMRInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, AthenaArray<Real> &prim_df,
                FaceField &b, Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh);
void DMROuterX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, AthenaArray<Real> &prim_df,
                FaceField &b, Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    nu_dust = pin->GetReal("dust", "nu_dust");
  }
      ...
      if (pcoord->x1f(i) > shock_xpos_top) {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            pdustfluids->df_u(rho_id, ks, j, i) = initial_D2G[n]*phydro->u(IDN,ks,j,i);
            pdustfluids->df_u(v1_id,  ks, j, i) = initial_D2G[n]*phydro->u(IM1,ks,j,i);
            pdustfluids->df_u(v2_id,  ks, j, i) = initial_D2G[n]*phydro->u(IM2,ks,j,i);
          }
        }

        if (NSCALARS > 0) {
          for (int n = 0; n<NSCALARS; n++)
            pscalars->s(n, ks, j, i) = phydro->u(IDN,ks,j,i);
        }
    ...
    for (int i=1;  i<=ngh; ++i) {
      ...
      if (NDUSTFLUIDS > 0) {
        for (int n = 0; n<NDUSTFLUIDS; n++) {
          ...
          prim_df(rho_id, kl, j, il-i) = initial_D2G[n]*d0;
          prim_df(v1_id,  kl, j, il-i) = u0;
          prim_df(v2_id,  kl, j, il-i) = v0;
          prim_df(v3_id,  kl, j, il-i) = 0.0;
        }
      }
    }
  }
      ...
      if (pco->x1v(i) < 0.1666666666) {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            prim_df(rho_id, kl, jl-j, i) = initial_D2G[n]*d0;
            prim_df(v1_id,  kl, jl-j, i) = u0;
            prim_df(v2_id,  kl, jl-j, i) = v0;
            prim_df(v3_id,  kl, jl-j, i) = 0.0;
          }
        }
      } else {
        // reflected
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            prim_df(rho_id, kl, jl-j, i) = prim_df(rho_id, kl, jl+(j-1), i);
            prim_df(v1_id,  kl, jl-j, i) = prim_df(v1_id,  kl, jl+(j-1), i);
            prim_df(v2_id,  kl, jl-j, i) = -prim_df(v2_id, kl, jl+(j-1), i);
            prim_df(v3_id,  kl, jl-j, i) = 0.0;
          }
        }
      }
      ...
      if (pco->x1v(i) < shock_pos) {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            prim_df(rho_id, kl, ju+j, i) = initial_D2G[n]*d0;
            prim_df(v1_id,  kl, ju+j, i) = u0;
            prim_df(v2_id,  kl, ju+j, i) = v0;
            prim_df(v3_id,  kl, ju+j, i) = 0.0;
          }
        }
      } else {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            prim_df(rho_id, kl, ju+j, i) = initial_D2G[n]*1.4;
            prim_df(v1_id,  kl, ju+j, i) = 0.0;
            prim_df(v2_id,  kl, ju+j, i) = 0.0;
            prim_df(v3_id,  kl, ju+j, i) = 0.0;
          }
        }
      }
