...
CXXFLAGS := -O3 -std=c++11 -fopenmp-simd -fwhole-program -flto -ffast-math -march=native -fprefetch-loop-arrays -fopenmp
...
SRC_FILES := $(wildcard src/*.cpp) \
	     ...
	     $(wildcard src/bvals/cc/dustdiffusion/*.cpp) \
	     $(wildcard src/bvals/cc/dustfluids/*.cpp) \
	     ...
	     src/coordinates/$(COORDINATES_FILE) \
			 $(wildcard src/dustfluids/*.cpp) \
			 $(wildcard src/dustfluids/dustfluids_diffusion/*.cpp) \
			 $(wildcard src/dustfluids/dustfluids_diffusion_cc/*.cpp) \
	     $(wildcard src/dustfluids/dustfluids_drags/*.cpp) \
	     $(wildcard src/dustfluids/srcterms/*.cpp) \
	     src/eos/general/$(GENERAL_EOS_FILE) \
	     src/eos/$(EOS_FILE) \
	     src/eos/eos_dustfluids.cpp \
